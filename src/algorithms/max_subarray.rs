...
pub fn max_sum_contiguous_naive(a: &[i64]) -> i64 {
    if a.is_empty() {
        return 0;
    }
...
pub fn max_sum_contiguous_kadane(a: &[i64]) -> i64 {
    ...
    for x in &a[1..] {
        let x = *x;
        prev_arr_sum = max(x, prev_arr_sum + x);
        max_sum = max(prev_arr_sum, max_sum);
    }
...
mod tests {
    ...
    fn contiguous() {
        ...
        assert_eq!(3 + 5 + 6 + 1, max_sum_contiguous_kadane(&a));
        assert_eq!(3 + 5 + 6 + 1, max_sum_contiguous_naive(&a));
    }

    #[test]
    fn simple() {
        let a: Vec<i64> = vec![1, 2, 3, 4];
        assert_eq!(10, max_sum_contiguous_kadane(&a));
        assert_eq!(10, max_sum_contiguous_naive(&a));
        ...
        let a: Vec<i64> = vec![];
        assert_eq!(0, max_sum_contiguous_kadane(&a));
        assert_eq!(0, max_sum_contiguous_naive(&a));
        assert_eq!(0, max_sum_non_contiguous(a));

        let a: Vec<i64> = vec![1];
        assert_eq!(1, max_sum_contiguous_kadane(&a));
        assert_eq!(1, max_sum_contiguous_naive(&a));
        assert_eq!(1, max_sum_non_contiguous(a));
        ...
        let a: Vec<i64> = vec![1, 2, 3];
        assert_eq!(6, max_sum_contiguous_kadane(&a));
        assert_eq!(6, max_sum_contiguous_naive(&a));
        ...
        assert_eq!(5, max_sum_contiguous_kadane(&a));
        assert_eq!(5, max_sum_contiguous_naive(&a));
        assert_eq!(6, max_sum_non_contiguous(a));
